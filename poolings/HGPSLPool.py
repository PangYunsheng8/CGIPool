...
class HGPSLPool(torch.nn.Module):
    def __init__(self, in_channels, ratio=0.8, sample=True, sparse=True, sl=True, lamb=1.0, negative_slop=0.2):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        if self.sample:
            ...
            new_edge_index, new_edge_attr = add_remaining_self_loops(new_edge_index, new_edge_attr, 0, x.size(0))  # 为啥还要self loop
            ...
            if self.sparse:
                ...
                new_edge_attr = softmax(weights, row)
            ...
            if edge_attr is None:
                induced_edge_attr = torch.ones((induced_edge_index.size(1), ), dtype=x.dtype, device=induced_edge_index.device)
            
            num_nodes = scatter_add(batch.new_ones(x.size(0)), batch, dim=0)  # compute num nodes of each graph
            ...
            for idx_i, idx_j in zip(shift_cum_num_nodes, cum_num_nodes):
                adj[idx_i:idx_j, idx_i:idx_j] = 1.0  # 为啥全给连起来了?
